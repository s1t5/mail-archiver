...
namespace MailArchiver.Services
{
    ...
    public class AuditExportService : BackgroundService, IAuditExportService
    {
        ...
        private readonly ConcurrentDictionary<Guid, CancellationTokenSource> _runningCancellations = new();
        ...
        public async Task<bool> CancelJobAsync(Guid jobId)
        {
            ...
            {
                // Cancel by job id, never whatever happens to be running: a stale Running row
                // from before a restart must not take down the export that is live right now.
                if (_runningCancellations.TryGetValue(jobId, out var cancellation))
                {
                ...
                {
                    // Nobody is executing this row (interrupted by a restart and not swept yet):
                    // close it here so the history shows the truth instead of Running forever.
                    job.Status = AuditExportJobStatus.Cancelled;
                    job.Completed = DateTime.UtcNow;
                    await context.SaveChangesAsync();
                    _logger.LogInformation("Cancelled stale audit data export job {JobId} that was left Running by a restart", jobId);
                }
                return true;
            }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            // Restart recovery: the queue is in memory only, so rows left Queued or Running by
            // a previous process would sit in that state forever (the polling UI would never
            // stop spinning). Queued rows are re-enqueued and still run; Running rows can never
            // be finished by this process and are closed as failed, their partial ZIP deleted.
            try
            {
            ...
            {
                _logger.LogWarning(ex, "Audit export restart recovery failed");
            }
        ...
        {
            // Every exit path must run the finally below: an early return that skips it would
            // leave the completion signal behind (stranding waiters) and leak the dictionary
            // entry for the lifetime of this singleton.
            try
            {
                ...
                // Registered before the row flips to Running: if the process dies between the
                // registration and the flip, the restart sweep still catches it as Queued;
                // if it dies after the flip, the sweep closes it as a failed interrupted run.
                _runningCancellations[jobId] = cancellation;
                ...
                {
                    _runningCancellations.TryRemove(jobId, out _);
                }
            }
            finally
            {
        ...
        /// <summary>
        /// Closes what a previous process left behind. Running rows cannot be finished by
        /// this process: they are marked failed, their partial ZIP (if any) deleted. Queued
        /// rows lost their in-memory queue slot — they are simply enqueued again and run.
        /// </summary>
        private async Task RecoverInterruptedJobsAsync()
        {
            ...
            var interrupted = await context.AuditExportJobs
                .Where(j => j.Status == AuditExportJobStatus.Queued || j.Status == AuditExportJobStatus.Running)
                .ToListAsync();
            ...
            foreach (var job in interrupted)
            {
                if (job.Status == AuditExportJobStatus.Running)
                {
                    ...
                    job.ErrorMessage = "Interrupted by an application restart while the export was running";
                    job.Completed = DateTime.UtcNow;
                    TryDeleteFile(job.OutputFilePath);
                    // A waiter (if one exists at all) must not keep waiting on a job that
                    // will never run. Queued rows keep their signal: the re-enqueued run
                    // fires it in ProcessJob's finally, and removing it here would strand
                    // whoever is waiting for the export to finish.
                    if (_completionSignals.TryRemove(job.Id, out var orphanedSignal))
                    {
                        orphanedSignal.TrySetResult();
                    }
                    _logger.LogInformation("Audit data export job {JobId} was left Running by a restart; marked as failed", job.Id);
                }
                else
                {
                    _logger.LogInformation("Re-enqueued audit data export job {JobId} that was left Queued by a restart", job.Id);
                }
            ...
            await context.SaveChangesAsync();
            _logger.LogInformation("Audit export restart recovery closed {Count} interrupted job(s)", interrupted.Count);
        }
